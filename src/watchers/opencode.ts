 ...
 */
function extractOpenCodeText(content: OpenCodeMessage["content"]): string {
  if (!content) return "";
 ...
 */
function parseOpenCodeSession(filePath: string): {
  content: string;
  messageCount: number;
  title?: string;
  projectPath?: string;
  ...
  const data = JSON.parse(raw) as OpenCodeSession | OpenCodeSession[];
 ...
 */
async function ingestOpenCodeSession(filePath: string): Promise<void> {
  try {
    ...
    const { content, messageCount, title, projectPath, sessionId } =
      parseOpenCodeSession(filePath);
    ...
    console.error(
      `[opencode-watcher] Sesión indexada: ${id} (${messageCount} msgs)`
    );
  } catch (err) {
...
export async function watchOpenCode(): Promise<void> {
  const existingPaths = OPENCODE_PATHS.filter((p) => fs.existsSync(p));

  if (existingPaths.length === 0) {
    console.error(
      "[opencode-watcher] No se encontraron directorios de OpenCode"
    );
    return;
  }
  ...
  for (const pattern of patterns) {
    chokidar
      ...
      .on("error", (err) =>
        console.error("[opencode-watcher] Error del watcher:", err)
      );
  }
}
