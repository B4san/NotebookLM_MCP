 ...
 */
async function ingestGeminiSession(filePath: string): Promise<void> {
  try {
    if (!filePath.endsWith(".json")) return;
    const stat = fs.statSync(filePath);
    if (stat.size < 10) return;

    const { content, messageCount, title, projectPath } =
    ...
    console.error(
      `[gemini-watcher] Sesión indexada: ${sessionId} (${messageCount} msgs)`
    );
  } catch (err) {
...
export async function watchGeminiCLI(): Promise<void> {
  if (!fs.existsSync(GEMINI_TMP_PATH)) {
    console.error(
      `[gemini-watcher] Directorio no encontrado: ${GEMINI_TMP_PATH}`
    );
    return;
  }
  ...
  chokidar
    ...
    .on("error", (err) =>
      console.error("[gemini-watcher] Error del watcher:", err)
    );
}
