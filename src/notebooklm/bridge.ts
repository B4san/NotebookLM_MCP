...
export class NotebookLMBridge {
  private static instance: NotebookLMBridge;
    ...
    return new Promise((resolve, reject) => {
      ...
      proc.on("close", (code) => {
        if (code !== 0) {
          reject(
            new Error(
              `notebooklm-py falló (código ${code}): ${stderr || stdout}`
            )
          );
        } else {
          resolve(stdout.trim());
      ...
      proc.on("error", (err) => {
        reject(
          new Error(
            `No se pudo ejecutar 'notebooklm'. Instala notebooklm-py: pip install "notebooklm-py[browser]". Error: ${err.message}`
          )
        );
      });
   ...
   */
  async checkAuth(): Promise<{ installed: boolean; authenticated: boolean; message: string }> {
    try {
  ...
  async listNotebooks(): Promise<Notebook[]> {
    try {
      ...
      const raw = await this.runJson<{ notebooks?: Notebook[]; items?: Notebook[] }>(
        ["list"]
      );
      const notebooks = raw.notebooks ?? raw.items ?? (Array.isArray(raw) ? raw as Notebook[] : []);
   ...
   */
  async getNotebook(notebookId: string): Promise<Notebook & { sources?: Source[] }> {
    await this.runCommand(["use", notebookId]);
    const meta = await this.runJson<{ notebook?: Notebook; sources?: Source[] }>(
      ["metadata"]
    );
  ...
  async listSources(notebookId: string): Promise<Source[]> {
    try {
      await this.runCommand(["use", notebookId]);
      const raw = await this.runJson<{ sources?: Source[] } | Source[]>(
        ["source", "list"]
      );
   ...
   */
  async searchSources(notebookId: string, query: string): Promise<ChatAnswer> {
    await this.runCommand(["use", notebookId]);
    const output = await this.runJson<{ answer?: string; text?: string; sources?: string[] }>(
      ["ask", query]
    );
    return {
  ...
  ): Promise<{ success: boolean; source_id?: string; message: string }> {
    await this.runCommand(["use", notebookId]);
   ...
   */
  async askNotebook(notebookId: string, question: string): Promise<ChatAnswer> {
    await this.runCommand(["use", notebookId]);
    const output = await this.runJson<{ answer?: string; text?: string; sources?: string[] }>(
      ["ask", question]
    );
