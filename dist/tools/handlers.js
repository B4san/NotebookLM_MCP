 ...
 */
import { listCliSessions, getCliSession as dbGetCliSession, searchCliSessions as dbSearchCliSessions, getSessionsByDateRange, } from "../storage/db.js";
...
// ─────────────────────────────────────────────────────────────────────────────
export async function handleListNotebooks(bridge) {
    try {
        const auth = await bridge.checkAuth();
        if (!auth.authenticated) {
            return errorResult("No autenticado con NotebookLM", auth.message);
        }
        const notebooks = await bridge.listNotebooks();
        if (notebooks.length === 0) {
            return textResult("📚 No tienes cuadernos en NotebookLM todavía.\n\nCrea uno en: https://notebooklm.google.com");
        }
        const list = notebooks
            .map((nb, i) => `${i + 1}. 📓 **${nb.title}**\n   ID: ${nb.id}\n   Fuentes: ${nb.source_count ?? "?"}`)
            .join("\n\n");
        return textResult(`📚 Tus cuadernos NotebookLM (${notebooks.length} total):\n\n${list}`);
    }
    catch (err) {
...
export async function handleGetNotebook(bridge, args) {
    try {
        const nb = await bridge.getNotebook(args.notebook_id);
        const sources = nb.sources ?? [];
        let text = `📓 **${nb.title}**\nID: ${nb.id}\n`;
        if (nb.created_at)
            text += `Creado: ${nb.created_at}\n`;
        if (nb.updated_at)
            text += `Actualizado: ${nb.updated_at}\n`;
        ...
        return errorResult(err instanceof Error ? err.message : String(err), `Verifica que el ID del cuaderno sea correcto: ${args.notebook_id}`);
    }
}
// ─────────────────────────────────────────────────────────────────────────────
// NotebookLM: Fuentes
// ─────────────────────────────────────────────────────────────────────────────
export async function handleListSources(bridge, args) {
    try {
        const sources = await bridge.listSources(args.notebook_id);
        if (sources.length === 0) {
            return textResult(`📭 El cuaderno ${args.notebook_id} no tiene fuentes.\n\nUsa \`add_source\` para agregar una.`);
        }
        const list = sources
            .map((s, i) => `${i + 1}. 📄 **${s.title ?? "Sin título"}**\n   ID: ${s.id}\n   Tipo: ${s.type ?? "desconocido"}${s.url ? `\n   URL: ${s.url}` : ""}`)
            .join("\n\n");
        return textResult(`📎 Fuentes del cuaderno (${sources.length} total):\n\n${list}`);
    }
    catch (err) {
        return errorResult(err instanceof Error ? err.message : String(err));
    }
}
export async function handleGetSourceContent(bridge, args) {
    try {
        const content = await bridge.getSourceContent(args.notebook_id, args.source_id);
        return textResult(`📄 Contenido de la fuente ${args.source_id}:\n\n${content}`);
    }
    catch (err) {
        return errorResult(err instanceof Error ? err.message : String(err));
    }
}
export async function handleSearchSources(bridge, args) {
    try {
...
export async function handleAddSource(bridge, args) {
    try {
        const result = await bridge.addSource(args.notebook_id, args.source, args.source_type);
        if (result.success) {
            return textResult(`✅ Fuente agregada exitosamente al cuaderno ${args.notebook_id}\n\n${result.message}`);
        }
        else {
...
// NotebookLM: Chat
// ─────────────────────────────────────────────────────────────────────────────
export async function handleAskNotebook(bridge, args) {
    try {
        ...
        return errorResult(err instanceof Error ? err.message : String(err), "Asegúrate de que el cuaderno tenga fuentes cargadas");
    }
}
// ─────────────────────────────────────────────────────────────────────────────
// Sesiones CLI
...
export async function handleListCliSessions(args) {
    ...
    if (sessions.length === 0) {
        return textResult("📭 No hay sesiones de CLIs indexadas aún.\n\nEl servidor monitorea automáticamente:\n" +
    ...
    const list = sessions
        ...
        return (`${i + 1}. ${emoji} **[${s.tool}]** ${s.title ?? "Sin título"}\n` +
            `   ID: ${s.id}\n` +
            `   Proyecto: ${s.project_path ?? "?"}\n` +
            `   Mensajes: ${s.message_count} · ${date}`);
    })
        .join("\n\n");
    return textResult(`🗂️ Sesiones CLI indexadas (${sessions.length}):\n\n${list}`);
}
export async function handleGetCliSession(args) {
    const session = dbGetCliSession(args.session_id);
    if (!session) {
        return errorResult(`Sesión no encontrada: ${args.session_id}`, "Usa list_cli_sessions para ver los IDs disponibles");
    }
    const date = new Date(session.timestamp).toLocaleString("es-MX");
    const header = `📋 Sesión: ${session.title ?? "Sin título"}\n` +
...
export async function handleExportSessionToNotebook(bridge, args) {
    ...
    try {
        ...
        const exportContent = `# ${title}\n\n` +
            ...
            `---\n\n` +
            session.content;
        const result = await bridge.addSource(args.notebook_id, exportContent, "text");
        if (result.success) {
            return textResult(`✅ Sesión exportada exitosamente a NotebookLM!\n\n` +
...
export async function handleGetActivityTimeline(args) {
    ...
    if (sessions.length === 0) {
        return textResult(`📅 No hay actividad en los últimos ${days} días.\n\nAsegúrate de que las CLIs estén siendo monitoreadas.`);
    }
    // Agrupar por día
    const byDay = new Map();
    ...
    for (const [day, daySessions] of byDay) {
        ...
        const byTool = daySessions.reduce((acc, s) => {
