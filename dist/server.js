...
import { handleListNotebooks, handleGetNotebook, handleListSources, handleGetSourceContent, handleSearchSources, handleAddSource, handleAskNotebook, handleListCliSessions, handleGetCliSession, handleSearchCliSessions, handleExportSessionToNotebook, handleGetActivityTimeline, } from "./tools/handlers.js";
// ─── Inicialización ────────────────────────────────────────────────────────
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        ...
        {
            name: "list_notebooks",
            description: "Lista todos tus cuadernos de NotebookLM con su ID, título y número de fuentes.",
            inputSchema: {
                type: "object",
                properties: {},
                required: [],
        ...
        {
            name: "get_notebook",
            description: "Obtiene los metadatos completos de un cuaderno NotebookLM específico (título, fuentes, estado).",
            inputSchema: {
                type: "object",
                properties: {
                    notebook_id: {
        ...
        {
            name: "list_sources",
            description: "Lista todas las fuentes (documentos, URLs, PDFs) de un cuaderno NotebookLM.",
            inputSchema: {
                type: "object",
                properties: {
                    notebook_id: {
        ...
        },
        {
            ...
            description: "Obtiene el contenido textual completo indexado de una fuente específica en NotebookLM. Ideal para leer el texto de un PDF o documento.",
            inputSchema: {
                type: "object",
                properties: {
                    notebook_id: {
        ...
        {
            name: "search_sources",
            description: "Busca en todas las fuentes de un cuaderno NotebookLM por palabras clave o pregunta en lenguaje natural.",
            inputSchema: {
                type: "object",
                properties: {
                    notebook_id: {
        ...
        {
            name: "add_source",
            description: "Agrega una nueva fuente (URL, ruta de archivo local, o texto) a un cuaderno NotebookLM.",
            inputSchema: {
                type: "object",
                properties: {
                    notebook_id: {
                    ...
                    source: {
                        type: "string",
                        description: "URL, ruta de archivo local (./file.pdf) o texto directo a agregar como fuente",
                    },
                    source_type: {
                        type: "string",
                        enum: ["url", "file", "text"],
        ...
        {
            name: "ask_notebook",
            description: "Hace una pregunta al cuaderno NotebookLM y obtiene una respuesta fundamentada en sus fuentes.",
            inputSchema: {
                type: "object",
                properties: {
                    notebook_id: {
        ...
        // ── Sesiones CLI ──
        {
            ...
            description: "Lista las sesiones guardadas de tus CLIs agénticos (Claude Code, Gemini CLI, OpenCode) con filtros por herramienta, proyecto o fecha.",
            inputSchema: {
                type: "object",
                properties: {
                    tool: {
        ...
        },
        {
            ...
            description: "Busca en el historial de todas tus sesiones CLI por palabras clave. Útil para encontrar cuando trabajaste en un tema específico.",
            inputSchema: {
                type: "object",
                properties: {
                    query: {
        ...
        },
        {
            ...
            description: "Exporta una sesión CLI como fuente a un cuaderno NotebookLM para que puedas consultarla con IA.",
            inputSchema: {
                type: "object",
                properties: {
                    session_id: {
        ...
        // ── Timeline ──
        {
            ...
            description: "Muestra una línea de tiempo unificada de tu actividad en todos los CLIs (qué hiciste, cuándo y en qué proyecto).",
            inputSchema: {
                type: "object",
                properties: {
                    days: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            ...
            case "get_notebook":
                return await handleGetNotebook(bridge, args);
            // NotebookLM: Fuentes
            case "list_sources":
                return await handleListSources(bridge, args);
            case "get_source_content":
                return await handleGetSourceContent(bridge, args);
            case "search_sources":
                return await handleSearchSources(bridge, args);
            case "add_source":
                return await handleAddSource(bridge, args);
            // NotebookLM: Chat
            case "ask_notebook":
                return await handleAskNotebook(bridge, args);
            // Sesiones CLI
            case "list_cli_sessions":
                return await handleListCliSessions(args);
            case "get_cli_session":
                return await handleGetCliSession(args);
            case "search_cli_sessions":
                return await handleSearchCliSessions(args);
            case "export_session_to_notebook":
                return await handleExportSessionToNotebook(bridge, args);
            case "get_activity_timeline":
                return await handleGetActivityTimeline(args);
            default:
...
async function main() {
    ...
    startWatchers().catch((err) => console.error("[watchers] Error al iniciar watchers:", err));
