 ...
 */
async function ingestClaudeSession(filePath) {
    try {
        const stat = fs.statSync(filePath);
        // Solo procesar archivos .jsonl con contenido
        if (!filePath.endsWith(".jsonl") || stat.size < 10)
            return;
...
export async function watchClaudeCode() {
    if (!fs.existsSync(CLAUDE_SESSIONS_PATH)) {
        console.error(`[claude-watcher] Directorio no encontrado: ${CLAUDE_SESSIONS_PATH}`);
        return;
    }
    ...
    chokidar
        ...
        .on("error", (err) => console.error("[claude-watcher] Error del watcher:", err));
}
